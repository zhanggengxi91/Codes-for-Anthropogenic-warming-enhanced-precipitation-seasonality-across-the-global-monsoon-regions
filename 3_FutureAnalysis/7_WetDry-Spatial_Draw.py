...
def plot(X, Y, trend, dir3):
    ...
    # 设置标题字体和大小
    title = "        " + title.replace("ssp126", "SSP1-2.6").replace("ssp245", "SSP2-4.5").replace("ssp585", "SSP5-8.5") \
            + " (" + dir3.split("_")[-1].replace(".nc", "") + " Season)"
    ax.set_title(title, font={'family': 'Times New Roman', 'weight': 'bold', 'size': 45}, loc="left", pad=20)
    colorbarRange = np.linspace(-5, 5, 11, endpoint=True)
    # Define the original colormap
    cmap_original = plt.cm.RdYlBu_r

    # Define the range of colors you want to extract
    start_value = 0.00  # Start position (between 0 and 1)
    end_value = 1  # End position (between 0 and 1)
    ...
    # cbar.ax.set_xlabel("Relative Change (mm/day)", fontsize=25)
    ...
    plt.savefig("G:/2_Monsoon_Distribution-Trend/6_WaterAvailability/6_WetDry_SpatialSraw/" + title.replace(" ","")  + '.png',
        format='png', transparent=True, dpi=100, bbox_inches='tight', pad_inches=0.2)
    plt.show()


if __name__ == '__main__':
    rootpath = "G:/2_Monsoon_Distribution-Trend/6_WaterAvailability/6_WetDry_SpatialSraw/Dry"
