...
# Other functions like extract_time_period() and plot() should be here

def calculate_ensemble_mean_cdo(path):
    nc_files = [f for f in os.listdir(path) if f.endswith(".nc") and not f.startswith("._")]
    input_files = [os.path.join(path, nc_file) for nc_file in nc_files]
    output_file = os.path.join(path, "enmean.nc")

    # Calculate ensemble mean using CDO
    subprocess.run(["cdo", "ensmean"] + input_files + [output_file])
    # cmd = "cdo ensmean " + input_files + " " + [output_file]
    # print(cmd)
    # os.system(cmd)
...
def extract_time_period(dataset, start_year, end_year):
    ...
    time_indices = np.where((time_var[:] >= start_date) & (time_var[:] <= end_date).astype(np.bool_))[0]
    return time_indices
...
def plot(X, Y, trend, p_value, dir2, dir3, rootpath, start_year, end_year):
    ...
    region_overlay = gpd.read_file("/mnt/f/PR_attributionPlot1_SpatialTrend/3_Trend/3_Draw_Picture1/PictureSpatial2_LatShift//World_Continents/World_Continents.shp")
    region_overlay.plot(ax=ax, color="none", edgecolor='black', zorder=100, alpha=0.8, linewidth=2.5)
    plt.savefig("/mnt/g/2_Monsoon_Distribution-Trend/1_Monsoon_SpatialTrend/" + title  + "_SI-SpatialTrend_"
                + str(start_year) + str(end_year) + '.png',
                format='png', transparent=True, dpi=100, bbox_inches='tight', pad_inches=0.2)
    plt.show()

if __name__ == '__main__':
    rootpath = "/mnt/g/2_Monsoon_Distribution-Trend/1_Monsoon_SpatialTrend/spatial"
    start_year = 1950
    end_year = 1980
    ...
    for dir2 in os.listdir(rootpath):
        ...
        if "observed" not in dir2:
            enmean_nc_path = calculate_ensemble_mean_cdo(path2)
            ncdata = nc.Dataset(enmean_nc_path)
        else:
