...
def plot(X, Y, trend, p_value, dir2, dir3, rootpath, start_year, end_year):
    ...
    print(dir2)
    if "observed" in dir2:
        title = str.upper(dir3.split("_")[3]) + " " + str(start_year) + "-"+str(end_year)
    elif "historical" in dir2:
        title = "ALL" + " " + str(start_year) + "-"+str(end_year)
    else:
        title = str.upper(dir2.replace("SI_hist-", "")) + " " + str(start_year) + "-"+str(end_year)
    print(title)
    ...
    p2 = ax.contourf(X, Y, p_value, [np.min(p_value), 0.05, np.max(p_value)], hatches=['.', None], zorder=100,
                     colors="none")
    # 设置colorbar
    # cbar = plt.colorbar(p1, ticks=colorbarRange, format='%.2f', orientation="horizontal", extend='Neither', aspect=80, fraction=0.49, pad=0.4)
    # cbar.set_label(' ', size=40, weight="bold")
    ...
    plt.savefig("G:/2_Monsoon_Distribution-Trend/1_Monsoon_SpatialTrend/6_DrawBoxplot/" + title.split(" ")[0] + "_SI-SpatialTrend_"
...
if __name__ == '__main__':
    rootpath = "G:/2_Monsoon_Distribution-Trend/1_Monsoon_SpatialTrend/6_DrawBoxplot/1950-2014"
    start_year = 1950
    end_year = 1979
