...
def get_data(path):
    ...
    time_num = len(ncdata["time"][0:7])
    x = np.arange(0, time_num)
    y = ncdata[zhishu][0:7, 0, 0] * 100
...
def get_data2(path):
    ncdata = nc.Dataset(path)
    zhishu = path.split("/")[-1].split("_")[1]
    time_num = len(ncdata["time"][7:13])
    x = np.arange(0, time_num)
    y = ncdata[zhishu][7:13, 0, 0] * 100
    slope, intercept, r, p, std_err = stats.linregress(x, y)
...
def get_draw_data2(infos):
    ...
    for info in infos:
        ...
        path = info.split("#")[1]
        if tag == "observed":
            observed2 = get_data2(path)
        if tag == "hist-aer":
            hist_aer2 = get_data2(path)
        if tag == "hist-GHG":
            hist_GHG2 = get_data2(path)
        if tag == "hist-nat":
            hist_nat2 = get_data2(path)
        if tag == "historical":
            historical2 = get_data2(path)
    return observed2, hist_aer2, hist_GHG2, hist_nat2, historical2

def plot(key, observed, hist_aer, hist_GHG, hist_nat, historical, obs1, hist_aer1, hist_GHG1, hist_nat1, historical1, observed2, hist_aer2, hist_GHG2, hist_nat2, historical2, xzhou1):
    # # 设置全局字体
    plt.rc('font', family='Times New Roman', size=25)
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(12, 5), gridspec_kw={'width_ratios': [9, 2,2], 'wspace': 0})
    ...
    ax3.spines['bottom'].set_linewidth(bwith)  # 图框下边
    ax3.spines['right'].set_linewidth(bwith)  # 图框左边
    ax3.spines['top'].set_linewidth(bwith)  # 图框上边
    ax3.spines['left'].set_visible(False)  # 图框右边

    ax1.tick_params(which="major", direction='out', length=7, width=1)
    # ax2.tick_params(which="major", direction='out', length=0, width=0)
    ...
    ax1.plot(xzhou1, obs1["z"], color="black", label='CRU', zorder=100, linewidth=2.0)
    ...
    ax2.set_xticks([3])
    ax2.set_xticklabels(["1950-1984"], rotation=0)

    ax3.set(ylim=(-1, 1))
    ax3.set_yticks([-1, -0.5, 0, 0.5, 1])
    ax3.axes.yaxis.set_ticklabels([])
    ax3.grid(color='grey', linestyle='--', linewidth=1, alpha=0.3)
    ax3.scatter([1, 1, 1], observed2, marker="_", color="grey", zorder=2, linewidth=1)
    ax3.vlines([1], observed2[0], observed2[2], color="grey", zorder=2, linewidth=1)
    ax3.bar([1], [observed2[1]], zorder=1, color="black")
    ax3.scatter([2, 2, 2], historical2, marker="_", color="grey", zorder=2, linewidth=1)
    ax3.vlines([2], historical2[0], historical2[2], color="grey", zorder=2, linewidth=1)
    ax3.bar([2], [historical2[1]], zorder=1, color="red")
    ax3.scatter([3, 3, 3], hist_GHG2, marker="_", color="grey", zorder=2, linewidth=1)
    ax3.vlines([3], hist_GHG2[0], hist_GHG2[2], color="grey", zorder=2, linewidth=1)
    ax3.bar([3], [hist_GHG2[1]], zorder=1, color="blue")
    ax3.scatter([4, 4, 4], hist_aer2, marker="_", color="grey", zorder=2, linewidth=1)
    ax3.vlines([4], hist_aer2[0], hist_aer2[2], color="grey", zorder=2, linewidth=1)
    ax3.bar([4], [hist_aer2[1]], zorder=1, color="darkorange")
    ax3.scatter([5, 5, 5], hist_nat2, marker="_", color="grey", zorder=2, linewidth=1)
    ax3.vlines([5], hist_nat2[0], hist_nat2[2], color="grey", zorder=2, linewidth=1)
    ax3.bar([5], [hist_nat2[1]], zorder=1, color="limegreen")
    ax3.set_xticks([3])
    ax3.set_xticklabels(["1985-2014"], rotation=0)


    plt.tight_layout()
    ax1.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),
               fancybox=True, shadow=True, ncol=5, prop={'size': 20, "family": "Times New Roman"})
    plt.savefig("G:/1_OrigionalData/4_AE_SpatialTrend/4_TimeSeries/Legend-" + key.replace("#", "_") + '.png',
...
if __name__ == '__main__':
    ...
    for key in file_info1.keys():
        ...
        observed2, hist_aer2, hist_GHG2, hist_nat2, historical2 = get_draw_data2(file_info[key])
        plot(key, observed, hist_aer, hist_GHG, hist_nat, historical, obs1, hist_aer1, hist_GHG1, hist_nat1, historical1, observed2, hist_aer2, hist_GHG2, hist_nat2, historical2,  xzhou1)
